// =============================================================================
// Register File Testbench — Verilator C++
// Testbench do Banco de Registradores — Verilator C++
// Tests: write/read, x0 always zero, dual-port read
...
int main(int argc, char** argv) {
    ...
    // ------------------------------------------------------------------
    // x0 is never modified (hardwired zero)
    // x0 nunca é alterado (hardwired zero)
    // ------------------------------------------------------------------
    printf("[ x0 hardwired zero ]\n");
    dut->we  = 1;
    dut->rd  = 0;       // attempt to write to x0 / tenta escrever em x0
    dut->wd  = 0xCAFEBABE;
    tick(dut, ctx);
    ...
    // ------------------------------------------------------------------
    // Simultaneous dual-port read / Leitura de dois registradores simultaneamente
    // ------------------------------------------------------------------
    ...
    // ------------------------------------------------------------------
    // Write to all 31 registers (x1–x31)
