...
// Validates: LW/SW (word), LBU/SB (unsigned byte), LB/SB (signed byte),
// Valida: LW/SW (word), LBU/SB (byte sem sinal), LB/SB (byte com sinal),
//         LHU/SH (unsigned half-word) / LHU/SH (half-word sem sinal)
// =============================================================================
#include <verilated.h>
...
int main(int argc, char** argv) {
    ...
    // 14 instructions + loop = 25 cycles is sufficient
    ...
    // x2 = 100 (read via LW / lido via LW)
    check(dut, 2, 100,        "lw mem[0]");
    // x3 = 0x55 (stored unsigned byte / byte sem sinal armazenado)
