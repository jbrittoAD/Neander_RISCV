...
//
// funct3:
//   000 = byte com sinal    (LB / SB)
//   001 = half com sinal    (LH / SH)
//   010 = word              (LW / SW)
//   100 = byte sem sinal    (LBU)
//   101 = half sem sinal    (LHU)
// =============================================================================
module data_mem #(
    ...
    output logic [31:0] rd       // Dado lido
);
    ...
    always_ff @(posedge clk) begin
        if (mem_write) begin
            case (funct3[1:0])
                ...
                2'b01: begin // Half-word (alinhado em 2 bytes)
                    if (!byte_off[1])
                        mem[widx][15:0]  <= wd[15:0];
                    else
                        mem[widx][31:16] <= wd[15:0];
                end
                2'b10: mem[widx] <= wd; // Word
                default: ; // ignorado
            endcase
        end
    end
    ...
    always_comb begin
        ...
        if (mem_read) begin
            case (funct3)
                3'b000: begin // LB — extensão de sinal
                    case (byte_off)
                ...
                3'b001: begin // LH — extensão de sinal
                    if (!byte_off[1])
                ...
                3'b100: begin // LBU — sem extensão de sinal
                    case (byte_off)
                ...
                3'b101: begin // LHU — sem extensão de sinal
                    if (!byte_off[1])
